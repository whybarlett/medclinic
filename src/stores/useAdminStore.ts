...
import { mockClinics, mockActions } from '../mocks/index'
...
export const useAdminStore = defineStore('admin', () => {
  ...
  async function saveClinic(clinic: Clinic) {
    ...
    else clinics.value.unshift({ ...clinic, id: Date.now() })
    isSaving.value = false
    flashSuccess()
  }
  ...
  async function saveAction(action: Action) {
    ...
    else actions.value.unshift({ ...action, id: Date.now() })
    isSaving.value = false
    flashSuccess()
  }
  ...
  // ── Настройки ──
  async function saveSiteSettings(settings: typeof siteSettings.value) {
    isSaving.value = true
  ...
  return {
    isAuthenticated, isLoading, isSaving, loginError, saveSuccess,
